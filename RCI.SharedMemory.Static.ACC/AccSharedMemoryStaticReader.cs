...
public class AccSharedMemoryStaticReader : IDisposable
{
  ...
  private readonly AccSharedMemoryStaticConfig _config;
  ...
  public AccSharedMemoryStaticReader(AccSharedMemoryStaticConfig? config = null)
  {
    _config = config ?? new AccSharedMemoryStaticConfig();
    _retryTimer = new Timer(ConnectInternal, null, Timeout.Infinite, Timeout.Infinite);

    if (_config.Physics.Enabled)
    {
      _physics = SharedMemoryStatic.Connect<Physics>(_config.Physics.UpdateInterval);
      _physics.Updated += physics => PhysicsUpdated?.Invoke(physics);
    }

    if (_config.Graphics.Enabled)
    {
      _graphics = SharedMemoryStatic.Connect<Graphics>(_config.Graphics.UpdateInterval);
    ...
    {
      _staticInfo = SharedMemoryStatic.Connect<StaticInfo>(_config.StaticInfo.UpdateInterval);
